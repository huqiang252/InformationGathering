...
if __name__ == '__main__':
    ...
    zhangsan.setHouseInfo('上地西里', 20, 5000, 1, '独立卫生间', 0)
    zhangsan.publishHouseInfo(myHome)

    lisi = HouseOwner('李四')
    lisi.setHouseInfo('当代城市花园', 16, 1800, 1, '独立卫生间', 0)
    lisi.publishHouseInfo(myHome)

    wangwu = HouseOwner('王五')
    wangwu.setHouseInfo('金踰美和园', 18, 2600, 1, '独立卫生间', 1)
    wangwu.publishHouseInfo(myHome)
    print()

    tony = Customer("Tony")
    houseInfos = tony.findHouse("有独立卫生间，面积大于15平米，价格小于2000", myHome)
    print()
    print('正在看房，寻找合适的住巢')
    print()
    houseInfo = tony.seeHouse(houseInfos)
    tony.signContract(houseInfo, myHome, 3)
